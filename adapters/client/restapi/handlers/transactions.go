...
func (h *Handler) Withdraw(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodPost:
		...
		if utils.GetRouteGroup(r) == "user" {
			...
			data["user_id"] = userID // Override user_id with the user_id from the context
		}

		service := h.appPort.NewTransactionsService()
		transaction, err := service.Withdrawal(data)
		if err, ok := err.(*utils.RequestError); ok {
			res, _ := templates.Template(w, err.StatusCode(), err.Error(), nil)
			w.Write([]byte(res))
			return
		...
		res, _ := templates.Template(w, http.StatusOK, "Withdrawal successful", transaction)
		w.Write([]byte(res))
	default:
		tmp, _ := templates.Template(w, http.StatusMethodNotAllowed, "Method not allowed", nil)
		w.Write([]byte(tmp))
