...
func (user *UserModel) PopulateUserModel(row *sql.Rows) error {
	var (
	...
	for row.Next() {
		if !isInitial { // If this is the first row, populate user model with the first row
			...
			if err := row.Scan(
				...
				&user.Account.Id, &user.Account.UserId, &user.Account.AccountTypeId, &user.Account.AccountNumber, &user.Account.Balance, &user.Account.CurrencyId,
				&user.AccountType.Id, &user.AccountType.Name,
				&user.Currency.Id, &user.Currency.Name, &user.Currency.Symbol,
			...
			user.UserPermissions = append(user.UserPermissions, permission)
			user.UserGroups = append(user.UserGroups, userGroup)
...
func (mAdapt *UserAdapter) Get(col string, value interface{}) (*UserModel, error) {
	...
	err = user.PopulateUserModel(rows)
	if err != nil {
		log.Fatal(err)
		return nil, err
	}
